...

{
  ...
  watch (status_t)
  {
    ...
    do
    {
      ...
      switch (thread -> info . status)
      {
        ...
        case DNA_THREAD_READY :
          {
            /*
             * Get the right queue for the job.
             */
            ...
            {
              queue = & scheduler . queue[thread -> info . affinity] . normal;
            }
    ...
    /*
     * Destroy the thread's alarm whatever its status is.
     */

    if (thread -> info . type == DNA_RT_THREAD)
    {
      alarm_destroy (thread -> periodic_alarm);
    }
